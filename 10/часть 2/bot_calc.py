...
def send_welcome(message):
    ...
    msg = bot.send_message(message.chat.id, 'Привет ' + message.from_user.first_name + ', я бот\nВведите число', reply_markup=markup)
...
def process_num1_step(message, user_result = None):
    try:
        ...
        bot.reply_to(message, 'Это не число или что-то пошле не так...')
...
def process_num2_step(message):
    try:
        ...
        msg = bot.send_message(message.chat.id, 'Показать результат или продолжить вычисление? ', reply_markup=markup)
